...
trait SimpleTestData extends DataReader {

  val readAsDF: DataFrame = {
    import sparkSession.implicits._

    val flatDataSchema = Seq("actor", "genre", "residence", "fee_per_movie", "age", "net_worth", "movie_count")

    Seq(("Gregory Peck", "Action", "USA", 100000, 58, 1000000, 100),
  ...
  val readAsDFWithSchema: DataFrame = {
    ...
    sparkSession.createDataFrame(sparkSession.sparkContext.parallelize(nestedData),
      readSchemaFromClassPath("actorBio.json"))
  }
